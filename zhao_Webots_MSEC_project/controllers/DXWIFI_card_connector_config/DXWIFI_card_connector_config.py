...
robot = Supervisor()
rospy.init_node("DXWIFI_CARD")
rate = rospy.Rate(20)
# get the time step of the current world.
timestep = int(robot.getBasicTimeStep())
connector_wifi_card = Connector("connector_wifi_card")
connector_wifi_card.enablePresence(timestep)
connector_cambera_board = Connector("connector_cambera_board")
connector_cambera_board.enablePresence(timestep)
...
iiwa2_wifi_card_pickup_notice = Connector("iiwa2_wifi_card_pickup_notice")
iiwa2_wifi_card_pickup_notice.enablePresence(timestep)
firsttime = True
solar_procedure=""
last_msg = ""
...
shape_node = robot.getFromDef("wificard_apperance")
...
while robot.step(timestep) != -1:
    ...
    #  val = ds.getValue()
    if connector_wifi_card.getPresence() == 1:
            connector_wifi_card.lock()
    if connector_cambera_board.getPresence() == 1:
            connector_cambera_board.lock()
            if (connector_cambera_board.getPresence() == 1):
    ...
    if (iiwa2_wifi_card_pickup_notice.getPresence() == 1):
